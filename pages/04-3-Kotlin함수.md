...
우리가 지금까지 흔히 써왔던 `main`과 `print`, `println`도 함수다.

저 두 개념은 프로그래밍 언어에서 함수로 구현한다.

함수를 만드는 방법은 다음과 같다.

`fun (함수 이름) (매개변수들): 반환타입 { 함수 본문 }`

- `fun (함수 이름) (매개변수들): 반환타입`: 함수 헤더
- `{ 함수 본문 }`: 함수 몸체
...
위 예시는 `String`에 내장된 함수를 호출하는 예시이다.
`.함수이름()`은 멤버 함수인 경우에만 이와 같은 방식으로
함수 호출이 가능하다.
...
```kotlin
fun printStringFunction(printString: String = "Print Function") {
    println(printString)
}
```

이렇게 하고 함수를 호출할 때 매개변수를 전달하지 않아도
기본 매개변수로 정한 값으로 출력이 된다.

### 2. 람다

람다(lambda)는 위 섹션에서 설명한 대로 함수를 만드는 것이 아닌
일회성으로만 사용할 명령문 모음을 만드는 것이다. 일회성으로 사용하는
코드들은 보통 코드를 정돈하지 않고 쓰는 경우가 있는데 코드가
길어질 경우 정돈하고 쓰는 게 가독성이 좋은 경우가 많다. 이때
람다가 유용해진다.
...
코틀린은 특이하게 매개변수로 함수를 전달 가능하다.
클릭을 했을 때 실행할 명령문들을 중괄호 내부에 작성하면 되는데,
지금은 아무것도 없으니 함수 자체는 실행되지만 아무런 일이 일어나지
않는다.
